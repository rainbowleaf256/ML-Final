...
class FracNetTrainDataset(Dataset):
    ...
    def __init__(self, image_dir, label_dir=None, crop_size=64,
        ...
        self.public_id_list = sorted([x.split("-")[0]
            for x in os.listdir(image_dir)])#接收绝对路径
        self.crop_size = crop_size
        self.transforms = transforms
        self.num_samples = num_samples
        self.train = train

    def __len__(self):#返回这个数据集一共有多少个item
        return len(self.public_id_list)

    @staticmethod
    def _get_pos_centroids(label_arr):
        centroids = [tuple([round(x) for x in prop.centroid])#四舍五入
                     for prop in regionprops(label_arr)]#regionprops：被标记的区域的面积分布
        return centroids
    ...
    def __getitem__(self, idx):#返回一条训练数据，并将其转换成tensor
    ...
    def collate_fn(samples):
        image_rois = torch.cat([x[0] for x in samples])#连接张量序列
